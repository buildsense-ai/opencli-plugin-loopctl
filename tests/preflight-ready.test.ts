...
function exactHistory(id:number|string, content:string, topic='grp_43', from=559){return {messages:[{id,seq_id:id,topic_id:topic,from_uid:from,type:'worker_ready',content:JSON.parse(content)}]}}
...
describe('preflight-ready Bot transport',()=>{
 ...
 it('finds the exact newest receipt in a bounded latest page and rejects a concurrent later message',async()=>{const root=mkdtempSync(join(tmpdir(),'preflight-bot-'));roots.push(root);install(root);let content='';const fetch=vi.fn(async(url:string,init:RequestInit)=>{if(url.endsWith('/api/me'))return response({uid:'559',account_type:'bot'});if(url.includes('/api/messages?'))return response({messages:[{id:1001,seq_id:1001,topic_id:'grp_43',from_uid:559,type:'worker_ready',content:JSON.parse(content)}]});const body=JSON.parse(String(init.body));content=body.content;return response({id:1001,seq_id:1001,topic_id:'grp_43',from_uid:559,client_msg_id:body.client_msg_id,duplicate:false})});vi.stubGlobal('fetch',fetch);await expect(invoke(root,packet())).resolves.toMatchObject({receipt:{seqId:'1001'}});expect(String(fetch.mock.calls[2][0])).toContain('latest=true');expect(String(fetch.mock.calls[2][0])).toContain('limit=100')
  const concurrent=vi.fn(async(url:string,init:RequestInit)=>{if(url.endsWith('/api/me'))return response({uid:'559',account_type:'bot'});if(url.includes('/api/messages?'))return response({messages:[{id:1001,seq_id:1001,topic_id:'grp_43',from_uid:559,type:'worker_ready',content:JSON.parse(content)},{id:1002,seq_id:1002,topic_id:'grp_43',from_uid:602,type:'text',content:'later'}]});const body=JSON.parse(String(init.body));content=body.content;return response({id:1001,seq_id:1001,topic_id:'grp_43',from_uid:559,client_msg_id:body.client_msg_id,duplicate:false})});vi.stubGlobal('fetch',concurrent);await expect(invoke(root,packet())).rejects.toThrow('not server-confirmed')
 })
 it('fails closed when the legacy around_id receipt endpoint errors',async()=>{const root=mkdtempSync(join(tmpdir(),'preflight-bot-'));roots.push(root);install(root);let content='';const fetch=vi.fn(async(url:string,init:RequestInit)=>{if(url.endsWith('/api/me'))return response({uid:'559',account_type:'bot'});if(url.includes('around_id='))return response({error:'failed to load messages'},500);if(url.includes('/api/messages?'))return response({messages:[{id:9,seq_id:9,topic_id:'grp_43',from_uid:559,type:'worker_ready',content:JSON.parse(content)}]});const body=JSON.parse(String(init.body));content=body.content;return response({id:9,seq_id:9,topic_id:'grp_43',from_uid:559,client_msg_id:body.client_msg_id,duplicate:false})});vi.stubGlobal('fetch',fetch);await expect(invoke(root,packet())).resolves.toMatchObject({receipt:{seqId:'9'}});expect(fetch.mock.calls.some(call=>String(call[0]).includes('around_id='))).toBe(false)})
