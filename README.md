...
Loop is opt-in, not the default behavior. For an ordinary human request after auth succeeds, Review remains a normal Agent and may answer, inspect, or modify through the host's usual workflow. Review enters Loop mode only when the human explicitly asks to use Loop, start a Loop, use `loopctl`, or otherwise requests Controller/Worker orchestration. Once Loop is selected, Review independently chooses a single Work Item or parallel fan-out based on dependency, write-scope overlap, isolation, and expected benefit; the Human does not need to prescribe the worker count. A Controller Action continues an already-started Loop; a structured group mention only wakes Review and does not select Loop by itself.
