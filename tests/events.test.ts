...
describe('strict Agent event builders', () => {
  ...
  it('accepts a fresh Project placeholder plan and rejects historical coordinator routes for agent-task start', () => {
    const registration={type:'work_item_registered',eventId:'r-start',idempotencyKey:'r-start',source:'review',entityRef:'work_item:w-start',payload:{workItemId:'w-start',loopId:'loop-1',profileId:'default',terminalState:'accepted',taskContractHash:'task-hash-1',referenceSnapshotHash:'ref-hash-1',writeScopeHash:'scope-hash-1',acceptanceContractHash:'accept-hash-1',writeScope:['src'],githubRepo:'org/repo',catscoProjectId:'project:new',workerTopicId:'agent-task:559',evidenceTopicId:'evidence-topic:559:574',stewardTopicId:'review-topic:574',stewardPrincipal:'catsco-user:574'}}
    ...
    expect(()=>parseAgentTaskStart(JSON.stringify([{...registration,payload:{...registration.payload,catscoProjectId:'project:auto'}},bundle]))).toThrow(/project:new/)
    expect(()=>parseAgentTaskStart(JSON.stringify([{...registration,payload:{...registration.payload,coordinatorSessionTopicId:'p2p_574_602'}},bundle]))).toThrow(/new Project coordinator/)
  })
