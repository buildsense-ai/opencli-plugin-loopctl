...
4. Decide the execution shape yourself after the Human opts into Loop. Use one Work Item and `opencli loop start --plan-file FILE` when the work is small, coupled, or cannot be safely isolated. Use `opencli loop fanout --plan-file FILE` only when Work Items have independent deliverables, non-overlapping or manageable write scopes, and enough expected benefit to justify parallelism. Do not fan out tasks that need ordered shared state, touch the same fragile files, or are cheaper to execute sequentially. For fan-out, create one registration/bundle pair per Work Item under the same `loopId`. Every bundle instruction must contain exactly one line beginning `LOOP_WORKTREE_CONTRACT_V1=` with JSON fields `repository`, `baseRevision`, `branchName`, `worktreePath`, `cleanupPolicy`, and `workspaceLease`. Keep Work Item IDs, Attempt IDs, branches, paths, and leases unique; bound fan-out to available Worker/runtime concurrency.
