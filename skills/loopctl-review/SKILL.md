...
6. Decide the execution shape yourself after the Human opts into Loop. Prefer bounded fan-out for large tasks when they can be decomposed into independent deliverables with non-overlapping or manageable write scopes. For any parallel plan, set every registration `catscoProjectId` to `project:auto`, set each `workerTopicId` to `agent-task:WORKER_UID`, set its `runtimePrincipal` to `catsco-user:WORKER_UID`, then run `opencli loop agent-task-fanout --plan-file FILE`. This command creates or reuses the owner-scoped `Loop <loopId>` CatsCo Project, provisions and verifies one isolated CatsCo Agent Task topic per Attempt, attaches and reads it back from that Project, then registers or dispatches anything. Do not use `opencli loop fanout` for concurrent Attempts sharing one Worker P2P topic. Use one Work Item and `opencli loop start --plan-file FILE` for small tasks, tightly coupled work, ordered shared state, fragile overlapping files, or work where parallel coordination would add more risk than value. Do not fan out merely to increase Worker count. An Agent UID is an addressable principal, not a single-session lock: the same Worker may serve concurrent, independent Attempts only through distinct dedicated Agent Task topics. Every bundle instruction must contain exactly one line beginning `LOOP_WORKTREE_CONTRACT_V1=` with JSON fields `repository`, `baseRevision`, `branchName`, `worktreePath`, `cleanupPolicy`, and `workspaceLease`. Keep Work Item IDs, Attempt IDs, generations, branches, paths, and leases unique; bound fan-out to the selected Worker's advertised runtime capacity.
